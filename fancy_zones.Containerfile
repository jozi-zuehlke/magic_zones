...
COPY src src
RUN dotnet publish src/FancyZonesPortable -r win-x64 --self-contained false -p:PublishSingleFile=true

FROM scratch
COPY --from=builder /fancy_zones/src/FancyZonesPortable/bin/Release/net8.0-windows/win-x64/publish/FancyZonesPortable.exe /fancy_zones/
